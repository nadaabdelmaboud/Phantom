...
export class UserService {
  ...
  async findByLogin(LoginDTO: LoginDTO): Promise<any> {
    ...
    const user = await this.userModel.findOne({ email: LoginDTO.email }).exec().then(async user => {
      return user ? user : 0;
    });;
    if (!user) throw new HttpException('not user by this email', HttpStatus.FORBIDDEN);
