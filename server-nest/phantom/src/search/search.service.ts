    ...
    constructor(
      ...
      async getMyPins(name, userId, limit, offset) {
        if (!this.ValidationService.checkMongooseID([userId]))
          ...
          let pins = await this.Fuzzy(pin,['title', 'note'], name, limit, offset);
          let myPins=[]
          for(let i=0;i<(await pins).length;i++){
            if(String(pins[i].creator.id)==userId)
            myPins.push(pins[i]);
      ...
      let users = await this.Fuzzy(user, ['firstName', 'lastName'], name, limit, offset);
      ...
      async getBoards(name, limit, offset) {
        ...
        let boards = await this.Fuzzy(board, ['description', 'status', 'topic'], name, limit, offset);
        return boards;
      }
