...
export class TopicService {
    ...
    async createTopic(imageId, description, imageWidth, imageHeight, name) {
        if (!this.ValidationService.checkMongooseID([imageId])) throw new Error('not mongoose id');
        let topicExist = await this.topicModel.find({'name': name}, (err, topicExist) => {
            if (err) return 0;
            return topicExist;
        });
    ...
    async getTopicById(topicId, userId): Promise<topic>{
        if (!this.ValidationService.checkMongooseID([topicId]))  throw new Error('not mongoose id');
    ...
    async addPinToTopic(topicName, pinId): Promise<Boolean>{
        if (!this.ValidationService.checkMongooseID([pinId])) throw new Error('not mongoose id');
        let topic = await this.topicModel.find({name: topicName}, async (err, topic) => {
            if (err) return await this.createTopic(undefined, undefined, undefined, undefined, topicName);
            return topic;
        });
    ...
    async getPinsOfTopic (topicId, limit, offset, userId): Promise<pin[]>{
        ...
        let pinsIds = await this.ValidationService.limitOffset(limit, offset, topic.pins);
        let pins = [];
