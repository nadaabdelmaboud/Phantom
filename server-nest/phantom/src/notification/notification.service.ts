...
export class NotificationService {
    ...
    async sendOfflineNotification(messages, fcmToken) {
        ...
        for (let i = 0; i < messages.length; i++)
            messages[i].token = fcmToken;

        app.messaging().sendAll(messages)
            .then((response) => {
    ...
    async followBoard(ownerUser, followerUser, boardName, boardId) {
        ...
        let message: {
            ...
            data: {
                ...
                body: followerUser.firstName + " " + followerUser.lastName + " has followed Your board " + '"' + boardName + '"'
            }
        };
        ...
        if (!ownerUser.fcmToken || ownerUser.fcmToken == ' ') {
            ...
            let checkFailed = await this.sendNotification([ownerUser.fcmToken], message);
            if (checkFailed) {
                message.tokens = null;
                ownerUser.offlineNotifications.push(message);
            }
    ...
    async commentPin(ownerUser, commenterUser, comment, pinName, pinId, imageId) {
        ...
        let message: {
            ...
            data: {
                ...
                body: commenterUser.firstName + " " + commenterUser.lastName + " has comment on your pin" + '"' + pinName + '"' + "say that" + comment
            }
        };
        if (!ownerUser.notifications) ownerUser.notifications = [];
        ownerUser.notifications.push(message);
    ...
    async reactPin(ownerUser, reactUser, pinName, pinId, react, imageId) {
        ...
        let message: {
            ...
            data: {
                ...
                body: reactUser.firstName + " " + reactUser.lastName + " has react on your pin" + '"' + pinName + '"'
            }
        };
        if (!ownerUser.notifications) ownerUser.notifications = [];
        ownerUser.notifications.push(message);
